...
## 可参考的截图
UIReference/PostNext.jpg


## 你当前的任务
设计PostNext页面，该页面的要素包括:
1. 页面上方左侧是返回箭头，右侧是“预览”按钮；接着是图片添加区域（显示上一步传入的图片），有已添加的图片和可添加图片的加号框；再往下是“添加标题”的输入区域；然后是上一步输入的文字的文字区域；之后是话题标签、@用户、投票等功能按钮区域；再往下依次是“标记地点”“仅自己可见”“添加组件”的设置区域；最底部左侧是“设置”和“存草稿”按钮，右侧是“发布笔记”按钮。
2. 仅自己可见权限可以更改，点击右侧箭头，出现如图UIReference/PostPrivacy.jpg的弹窗进行选择
3. 该界面的进入位置是“底部加号->下一步”
4. 适当修改PostTab，使其点击相机图库图片上传
5. 尽可能与参考截图一致