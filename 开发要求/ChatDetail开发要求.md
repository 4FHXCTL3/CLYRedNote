...
## 可参考的截图
UIReference/ChatDetail.jpg


## 你当前的任务
设计ChatDetail页面，该页面的要素包括:

1. 顶部信息栏：左侧返回箭头按钮（点击可返回），对方备注名居中，右侧三点
2. 中部聊天框：
    2.1：好友头像左侧（如果是群，显示说话人头像，头像上方显示发言人昵称小字），本人头像右侧
    2.2：发言内容，直接显示聊天信息
    2.3 最近发言时间（直接简化成MM.DD HH:MM即可，不必显示成“昨天”、“早晨”等相对时间，也不用每条信息上方都显示）

3. 底部输入框可输入文字，点击输入框可出现输入键盘进行文字输入，点击加号可调用系统图像库选择图片发送，从底部发送的信息立即出现在中部聊天框中

4. 要求从ChatTab点击相应人或群聊可以进入该ChatDetail界面