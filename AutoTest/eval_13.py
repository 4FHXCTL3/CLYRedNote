...
def share_note_check(result=None, device_id=None,backup_dir=None):
    ...
    try:
        ...
        if not data or len(data) == 0:
            ...
            print(f"   Expected: At least one share record for user '{USER_ID}'")
            return False

        # 查找用户的分享记录
        user_shares = [item for item in data if item.get('userId') == USER_ID]
        ...
        print(f"   Reason: User '{USER_ID}' has not shared any notes")
